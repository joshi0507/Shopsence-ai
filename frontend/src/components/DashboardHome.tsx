...
const DashboardHome = memo(
  ({
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        try {
          const [uploadsRes, kpisRes, chartsRes] = await Promise.all([
            ...
            api.getCharts(uploadId, timePeriod),
          ]);
      ...
      fetchData();
    }, [uploadId, refreshKey, timePeriod]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 glass p-6 rounded-2xl border border-white/10">
            <div className="flex items-center justify-between mb-6">
              ...
              <select
                className="bg-white/5 border border-white/10 rounded-lg text-xs text-gray-400 px-2 py-1 outline-none cursor-pointer hover:bg-white/10 transition-colors"
                value={timePeriod || ""}
                onChange={(e) => {
                  const value = e.target.value;
                  setTimePeriod(value ? parseInt(value) : undefined);
                }}
              >
