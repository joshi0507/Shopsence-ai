...
const Dashboard = ({ user, onLogout }: DashboardProps) => {
  ...
  // Fetch user progress and latest upload
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const res = await api.getUploads();
        if (res.success && res.data) {
          // Sort by created_at to get the most recent first
          const sortedByDate = [...res.data].sort(
            (a, b) =>
              new Date(b.created_at).getTime() -
              new Date(a.created_at).getTime(),
          );
          const completed = sortedByDate.filter(
  ...
  const handleNavigate = (tab: string) => {
    ...
    // Refresh overview data when navigating to dashboard
    if (tab === "dashboard") {
  ...
  const handleRefreshOverview = () => {
    // Increment refresh key to force DashboardHome to re-fetch data
    setRefreshKey((prev) => prev + 1);
    // Also refresh the latest upload ID
  ...
  const handleBackFromReport = () => {
    ...
    // Refresh the latest upload ID when returning from a report
    handleRefreshOverview();
  };
